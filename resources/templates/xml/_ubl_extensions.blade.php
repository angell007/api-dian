            ...
            <sts:DianExtensions>
                ...
                <sts:InvoiceSource>
                    <cbc:IdentificationCode listAgencyID="6"
                        ...
                        {{ $company->country->code }}</cbc:IdentificationCode>
                </sts:InvoiceSource>
                @php
                ...
                <sts:SoftwareProvider>
                    <sts:ProviderID schemeAgencyID="195"
                        ...
                        @if ($company->type_document_identification_id == 6 && !empty($companyDv)) schemeID="{{ $companyDv }}" @endif
                        schemeName="{{ $company->type_document_identification->code }}">{{ $companyIdentification }}
                    </sts:ProviderID>
                    <sts:SoftwareID schemeAgencyID="195"
                        schemeAgencyName="CO, DIAN (Direcci&#xF3;n de Impuestos y Aduanas Nacionales)">
                        {{ $company->software->identifier }}</sts:SoftwareID>
                </sts:SoftwareProvider>
                ...
                <sts:QRCode>https://catalogo-vpfe.dian.gov.co/document/searchqr?documentkey={{ $cufe_propio }}
                </sts:QRCode>
            </sts:DianExtensions>
        </ext:ExtensionContent>
    </ext:UBLExtension>
