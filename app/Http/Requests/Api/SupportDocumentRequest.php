...
class SupportDocumentRequest extends FormRequest
{
    ...
    {
        $this->resolution = auth()->user()->company->resolutions->where('type_document_id',"=", $this->type_document_id)->where('id','=', $this->resolution_id)->first();
        return [
            // Document
            'type_document_id' => [
                'required',
            ...
            'customer.type_liability_id' => 'required|exists:type_liabilities,id',
            ...
            'origin_reference' => 'nullable|array',
            'origin_reference.code' => 'nullable|string',
            'origin_reference.date' => 'nullable|date_format:Y-m-d',
            ...
            'allowance_charges.*.discount_id' => 'nullable',
            ...
            'invoice_lines.*.note' => 'required|string',
            'invoice_lines.*.brand_name' => 'nullable|string',
            'invoice_lines.*.model_name' => 'nullable|string',
            'invoice_lines.*.pack_size_numeric' => 'required|numeric',


            'invoice_lines.*.sellers_item_identification' => 'nullable|array',
            'invoice_lines.*.sellers_item_identification.id' => 'nullable|string',
            'invoice_lines.*.sellers_item_identification.code' => 'nullable|string',
 
            'invoice_lines.*.invoice_period' => 'required|array',
            'invoice_lines.*.invoice_period.description_code' => 'required|string',
            'invoice_lines.*.invoice_period.date' => 'required|date_format:Y-m-d',
            'invoice_lines.*.invoice_period.description' => 'required|string',
            ...
            'invoice_lines.*.withholding_tax_totals' => 'nullable|array',
            'invoice_lines.*.withholding_tax_totals.*.tax_amount' => 'nullable|required_with:invoice_lines.*.withholding_tax_totals|exists:taxes,id',
            'invoice_lines.*.withholding_tax_totals.*.tax_amount' => 'nullable|required_with:invoice_lines.*.withholding_tax_totals|numeric',
            'invoice_lines.*.withholding_tax_totals.*.taxable_amount' => 'nullable|required_with:invoice_lines.*.withholding_tax_totals|numeric',
            'invoice_lines.*.withholding_tax_totals.*.percent' => 'nullable|required_unless:invoice_lines.*.withholding_tax_totals.*.tax_id,10|numeric',
            'invoice_lines.*.withholding_tax_totals.*.unit_measure_id' => 'nullable|required_if:invoice_lines.*.withholding_tax_totals.*.tax_id,10|exists:unit_measures,id',
            'invoice_lines.*.withholding_tax_totals.*.per_unit_amount' => 'nullable|required_if:invoice_lines.*.withholding_tax_totals.*.tax_id,10|numeric',
            'invoice_lines.*.withholding_tax_totals.*.base_unit_measure' => 'nullable|required_if:invoice_lines.*.withholding_tax_totals.*.tax_id,10|numeric',
