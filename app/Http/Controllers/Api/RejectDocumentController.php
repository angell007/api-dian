...
use App\Http\Requests\Api\ReceiptGoodOrServiceRequest;
...
use Carbon\Carbon;
use App\Http\Requests\Api\RejectDocumentRequest;
...
class RejectDocumentController extends Controller
{
     ...
     * Store.
     *
     * @param \App\Http\Requests\Api\RejectDocumentRequest $request
     *
     * @return \Illuminate\Http\Response
     */
    public function store(RejectDocumentRequest $request)
    {
        // User
 
        $environment = RejectDocumentController::envirom['production'];
        ...
        $typeRejection = TypeRejection::findOrFail($request->type_rejection_id);
        ...
        $typeDocument = TypeDocument::findOrFail($request->type_document_id);
          ...
          $notification = $this->createXML(
            compact('user','typeDocument'
                    ...
                    'time','date','cude_propio','code','environment','person','typeRejection' ));
         // Signature XML
     ...
     * @return \Illuminate\Http\Response
     */
    public function testSetStore(RejectDocumentRequest $request, $testSetId)
    {
     

        $environment = RejectDocumentController::envirom['debug'];
        ...
        $notification = $this->createXML(
          compact('user','typeDocument'
                  ,'company' ,'reference', 'resolution','customer',
                  'time','date','cude_propio','code','environment' ));
       ...
       // Signature XML
        $signNotification = new SignExpressAcceptance($company->certificate->path, $company->certificate->password );
        $signNotification->typeFile = 'ExpressAcceptance';
        $signNotification->softwareID = $company->software_nomina->identifier;
        $signNotification->pin = $company->software_nomina->pin;
        ...
        //$SendExpressAcceptanceAsync = new SendPayrollAsync($company->certificate->path, $company->certificate->password);
        $sendExpressAcceptanceAsync->To = $company->software_nomina->url;
        $sendExpressAcceptanceAsync->fileName = "nie{$request->file}.xml";
        $sendExpressAcceptanceAsync->contentFile = $this->zipBase64Payroll($company, $signNotification->sign($notification),$request->file);
