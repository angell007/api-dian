<Devengados>
    ...
    <Vacaciones>
        ...
        <VacacionesComunes
            @if ( isset(  $accrued['vacations']['cumn']['date_start'] ) )
                FechaInicio="{{ $accrued['vacations']['cumn']['date_start'] }}"
            @endif
            @if ( isset(  $accrued['vacations']['cumn']['date_end'] ) )
                FechaFin="{{ $accrued['vacations']['cumn']['date_end'] }}"
            @endif
            Cantidad="{{ $accrued['vacations']['cumn']['days'] }}" Pago="{{ $accrued['vacations']['cumn']['value'] }}" />
        @endif
    ...
    <Primas Cantidad="{{$accrued['prima']['days']}}"
        @if ( isset( $accrued['prima']['value'] ) )
            Pago="{{$accrued['prima']['value']}}"

        @endif
        @if ( isset( $accrued['prima']['value_ns'] ) )
            PagoNS="{{$accrued['prima']['value_ns']}}"
        @endif
        />
    @endif
    ...
    <Licencias>
        @if ( isset( $accrued['licences']['mp'] ))
            ...
            <LicenciaMP FechaInicio="{{ $mp['date_start'] }}" FechaFin="{{ $mp['date_end'] }}" Cantidad="{{ $mp['days'] }}" Pago="{{ $mp['value'] }}" />
            @endforeach
        @endif
        @if ( isset( $accrued['licences']['r'] ))
            @foreach ($accrued['licences']['r'] as $r)
            <LicenciaR FechaInicio="{{ $r['date_start'] }}" FechaFin="{{ $r['date_end'] }}" Cantidad="{{ $r['days'] }}" Pago="{{ $r['value'] }}" />
            @endforeach
        @endif
        @if ( isset( $accrued['licences']['nr'] ))
            @foreach ($accrued['licences']['nr'] as $nr)
            <LicenciaNR FechaInicio="{{ $nr['date_start'] }}" FechaFin="{{ $nr['date_end'] }}" Cantidad="{{ $nr['days'] }}" />
            @endforeach
        @endif
    </Licencias>
    @endif
